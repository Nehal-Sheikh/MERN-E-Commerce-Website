...
exports.getProducts = (req, res, next) => {
    ...
    .then(products => {
        res.render('shop/product-list', {prods: products, pageTitle: 'All Products', path: '/products', isAuthenticated: req.session.isLoggedIn});          //in this 'shop/product-list' link the shop is the folder in the views folder and the product-list is the file in the shop folder
    })
    .catch(err => {
        console.log(err)
    })
...
exports.postOrder = (req, res, next) => {
    ...
    .then(user => {
        ...
        const order = new Order({                                                                 //here we are creating new order Object by using order model constructor
            user: {
                name: req.user.name,                                                              //here req.user gives the complete user object fetched from the database so it haves all the data about the user
                userId: req.user
            },
            products: products
        });
